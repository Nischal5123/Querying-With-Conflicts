...
# Example CLI
# =============================================================================
def load_amazon_products_df():
    candidate = "../data/real/amazon_products.csv"
    if os.path.exists(candidate):
        try:
            df = pd.read_csv(candidate)
            # --- Most Reasonable Binning for Price ---
            if "price" in df.columns:
                print("\n--- Processing 'price' column ---")

                # 1. Clean the data: ensure it's numeric and has no missing values
                df["price"] = pd.to_numeric(df["price"], errors="coerce")
                df.dropna(subset=["price"], inplace=True)

                # 2. Apply equi-depth (quantile) binning using pd.qcut.
                # This is the most reasonable method for skewed data like price because it ensures
                # each bin has an equal number of products, making the bins statistically meaningful.
                num_bins = 10  # Using 10 bins (deciles) provides good granularity.

                print(f"Applying equi-depth (quantile) binning with {num_bins} bins...")

                # We create a new column first to inspect the results
                binned_prices = pd.qcut(df['price'], q=num_bins, labels=False, duplicates='drop')

                # 3. Inspect the distribution (the counts should be balanced)
                print("\nDistribution of products into bins:")
                print(binned_prices.value_counts().sort_index())

                # 4. Replace the original 'price' column with the new bin IDs
                df['price'] = binned_prices
                print("\n✅ Successfully replaced 'price' column with bin IDs.")

            # --- Standard processing for other columns ---
            if "stars" in df.columns:
                df["stars"] = pd.to_numeric(df["stars"], errors="coerce").dropna().astype(int)
            return df, True
        except Exception:
            pass
    # Synthetic fallback
...
if __name__ == "__main__":
    ...
    df, found = load_amazon_products_df()

    candidate_cols = ["price","stars","boughtInLastMonth"]
    ...
    print(f"Done. CSV -> ../results/tables/{dataset}_more_time_vs_num_attrs.csv")
    print(f"Plot -> ../results/plots/{dataset}_more_time_vs_num_attrs.png")
