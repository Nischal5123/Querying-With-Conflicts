...
- Experiment:
    * run_scalability_time_vs_num_attrs_logging(...)
      ...
      - Bias: random multilevel over the tuple domain
      - Measure:
...
def run_scalability_time_vs_num_attrs_logging(
    ...
    *,
    ...
    alpha: float = 0.5,
    bias_levels: Tuple[float, ...] = (0.6, 0.5, 0.2, 0.2),
    ...
    for k in range(1, max_k + 1):
        ...
        bias_obj = make_random_multilevel_bias(dom, levels=bias_levels, probs=bias_probs, seed=bias_seed)
        biases   = biases_from_bias_obj(dom, bias_obj)

        # --- Alg.1 first pass (on singleton query) ---
        ...
        q_base = algorithm_2_build_qbase(
            dom, dom, prior,
            biases=biases, receiver_model="threshold", gamma=1.0, eps_order=0.0,
            rule=rule, boundary_policy=boundary_policy, alpha=alpha
        )
        ...
        time_alg24 = time_alg2 + time_alg4  # for plotting
    ...
    # Plot (combined time only)
    if xs:
        plt.figure(figsize=(5.2, 3.0))
        #make millisecond
        ys_alg1=[ y*1000 for y in ys_alg1]
        ys_alg24=[ y*1000 for y in ys_alg24]

        plt.plot(xs, ys_alg1, marker="x", linestyle="-", label="Credibility Detection")
        plt.plot(xs, ys_alg24, marker="o", linestyle="-", label="Maximally Informative")
...
# Example CLI
# =============================================================================
def load_pricerunner_df():
    candidate = "../data/real/pricerunner_simplified.csv"
    if os.path.exists(candidate):
        try:
            df = pd.read_csv(candidate)
            mostfreq = df["brand"].value_counts().nlargest(10).index.tolist()
            df["brand"] = df["brand"].apply(lambda x: x if x in mostfreq else "Other")
            mostfreq = df["MerchantID"].value_counts().nlargest(10).index.tolist()
            df["MerchantID"] = df["MerchantID"].apply(lambda x: x if x in mostfreq else "Other")
            return df, True
        except Exception:
            pass
    #Synthetic fallback
    rng = np.random.default_rng(123)
    #pick more frequent 50 brands then map other to others
...
if __name__ == "__main__":
    # Minimal demo with a synthetic dataset.
    # Replace with your real dataframe if desired.
    dataset="pricerunner"
    df, found = load_pricerunner_df()

    # Attribute sets
    candidate_cols= ["brand","MerchantID","CategoryID"]
    useless=["flight","price","duration","stops","departure_time","arrival_time","duration","datys_left"]
    candidate_cols = [c for c in candidate_cols if c not in useless]
    #print# age has too many distinct values
    ...
    run_scalability_time_vs_num_attrs_logging(
        ...
        total_timeout_s=600.0,  # 10 minutes budget
        csv_path=f'../results/tables/{dataset}_time_vs_num_attrs.csv',
        plot_path=f'../results/plots/{dataset}_time_vs_num_attrs.png',
        append=False,
        rule="paper",
        boundary_policy="all",
        alpha=0.5,
        bias_levels=(0.6, 0.5, 0.2, 0.2),
